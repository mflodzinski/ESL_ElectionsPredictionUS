...
result = pyreadr.read_r('Data/df_pid.RData') 
df_miss = result["df_miss"] 
df_test = result["df_test"] 
df_train= result["df_train"]

y_train = df_train['pid_election_returns']
X_train = df_train.drop(['pid_election_returns', 'data_id'], axis=1)
y_test = df_test['pid_election_returns']
X_test = df_test.drop(['pid_election_returns', 'data_id'], axis=1)
y_test.dtypes
...
X = df_miss.drop(['pid_election_returns', 'data_id'], axis=1)
f = X.copy()
...
# mod = pickle.load(open('Model/1_rf_wrace.pkl', 'rb'))
