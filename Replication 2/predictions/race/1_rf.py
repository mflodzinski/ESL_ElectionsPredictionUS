...
result  = pyreadr.read_r('Data/df_race.RData')
df_miss = result["df_miss"] 
df_test = result["df_test"] 
df_train= result["df_train"]

df_test['trueY']  = df_test['mturk_race'].map({'asian':0, 'black':1, 'caucasian':2, 'hispanic':3, 'other':4})
df_train['trueY'] = df_train['mturk_race'].map({'asian':0, 'black':1, 'caucasian':2, 'hispanic':3, 'other':4})

y_train = df_train['trueY']
X_train = df_train.drop(['trueY', 'mturk_race', 'data_id'], axis=1)
y_test  = df_test['trueY']
X_test  = df_test.drop(['trueY', 'mturk_race', 'data_id'], axis=1)
...
d[['predY0','predY1','predY2','predY3','predY4']] = grid_rf.best_estimator_.predict_proba(X_test)
...
e[['predY0','predY1','predY2','predY3','predY4']] = grid_rf.best_estimator_.predict_proba(X_train)
e['predY'] = grid_rf.best_estimator_.predict(X_train)
e['grp'] = 'train'

X = df_miss.drop(['mturk_race', 'data_id'], axis=1)
f = X.copy()
f['data_id'] = df_miss['data_id']
f.reset_index(inplace=True)
f[['predY0','predY1','predY2','predY3','predY4']] = grid_rf.best_estimator_.predict_proba(X)
